//
//  PCWaitPayOrderDetailViewController.m
//  BoMuCai
//
...
@implementation PCWaitPayOrderDetailViewController
...
- (void)cancelDelRequest
{
    ...
    [request requestWithSuccess:^( BXHBaseRequest *request) {
        ...
        if ([request.response.code isEqualToString:@"0000"])
        {
            [weakSelf.protcol waitPayDetailControllerDelOrder:weakSelf];
...
- (void)bottomView:(PCOrderDetailBtnBottomView *)bottomView btnActionAtIndex:(NSInteger)index
{
    ...
    else
    {
        ...
        payModel.amount = self.detailModel.amount;
        payModel.realAmount = self.detailModel.realAmount;
        payModel.yunfei = self.detailModel.yunfei;
        payModel.orderId = self.detailModel.orderId;
        payModel.couponPrice = self.detailModel.couponPrice;
...
- (PCOrderDetailStatueTimeCell *)statueCell
{
    if (!_statueCell)
    {
        ...
        NSString *str = @"订单状态：待付款";
        _statueCell.statueLabel.attributedText = [str attributStrWithTargetStr:@"待付款" color:Color_Main_Dark];
    }
    return _statueCell;
}
...
- (PCOrderDetailStatueCell *)secStatueCell
{
    if (!_secStatueCell)
    {
        ...
        NSString *str = @"订单状态：确认收货";
        _secStatueCell.statueLabel.attributedText = [str attributStrWithTargetStr:@"确认收货" color:Color_Main_Dark];
    }
...
- (PCOrderDetailBtnBottomView *)bottomView
{
    if (!_bottomView)
    {
        _bottomView = [[PCOrderDetailBtnBottomView alloc] initWithTitles:@[@"取消订单",@"付款"]];
