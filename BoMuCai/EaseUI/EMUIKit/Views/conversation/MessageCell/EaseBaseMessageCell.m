...
#pragma mark - request
- (void)requestWithShopId:(NSString *)shopId
{
    [self.request cancel];
    BXHWeakObj(self);
    ...
    [self.request requestWithSuccess:^( BXHBaseRequest *request) {
        if ([request.response.code isEqualToString:@"0000"])
        {
            ...
            [selfWeak.avatarView sd_setImageWithURL:[NSURL encodeURLWithString:request.response.data[@"image"]] placeholderImage:ImagePlaceHolder];
        }
...
//    _nameLabel.text = model.nickname;
    ...
    if (self.model.isSender) {
        [self.avatarView sd_setImageWithURL:[NSURL encodeURLWithString:KAccountInfo.photo] placeholderImage:model.avatarImage];
        self.nameLabel.text = KAccountInfo.nickName;
        _hasRead.hidden = YES;
    ...
    {
        if ([[self.model nickname] isEqualToString:@"test"])
        {
            self.avatarView.image = ImageWithName(@"SysMessageIcon");
            self.nameLabel.text = @"系统客服";
        }
        else
        {
            [self requestWithShopId:[self.model nickname]];
        }
    }
}
